...
class MainClass:
    ...
    def run(self) -> None:
        ...
        writer = cv2.VideoWriter(self.output_filename, fourcc, self.fps, (1920, 1080))
        ...
        for fno in tqdm(range(self.frame_count)):
            ...
            logger.debug(f"Frame {fno}: read in frame in {read_end - read_start} seconds")
            if not ret:
                break
            ...
            if fno % self.frames == 0:
                ...
                logger.debug(f"Frame {fno}: Took {mask_end - mask_start} seconds to generate mask")
                ...
                for c in contours:
                    ...
                    if 25 < x and x < 1895 and 25 < y and y < 1055:
                        ...
                        if 150 < area < 1000 and circle_area * .5 < area < circle_area * 1.5:
                            ...
                            image = frame[np.ix_(range(y - 25, y + 25),range(x - 25, x + 25))][:,:,::-1]
                            ...
                            evaluated_contours.append(Contour(x, y, radius, ss_chance))
                ...
                logger.debug(f"Frame {fno}: Took {contour_end - contour_start} seconds to evaluate contours")
            else:
            ...
            for i, p in enumerate(past_positions[:-1]):
                if p is not None:
                    cv2.circle(frame, (p.x, p.y), p.radius // 2, (0, 255, 0), -1)
            ...
            logger.debug(f"Frame {fno}: Took {write_end - write_start} seconds to write frame")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filename", "-f", required=True, help="The filename of the video to process")
    parser.add_argument("--color", "-c", help="The color to trace the ball with")
    parser.add_argument("--debug", "-d", action="store_true", help="Enable debug output")
    parser.add_argument("--nn", "-n", required=True, help="The path to the neural network to use")
    parser.add_argument("--display", action="store_true", help="Display output to screen frame by frame as each frame is loaded")
    parser.add_argument("--frames", type=int, default=1, help="Only frame numbers divisible by this number will be processed (other frames will not be analyzed)")
    parser.add_argument("--output", "-o", required=True, help="The path to the output file")
